...
If you are familiarized with [tidyverse](https://www.tidyverse.org/) you can use any of the usual `dplyr` commands in you database tables.
...
\*Please note this function will be renamed to **dropSourceTable**
...
```{r, error = TRUE}
cdm$person <- cdm$drug_exposure |> compute(name = "person")
```

. . .

```{r, error = TRUE}
cdm$drug_exposure <- cdm$drug_exposure |> rename("my_id" = "person_id") |> compute(name = "drug_exposure")
```
